import { For, useContext, createResource } from "solid-js"
...
export default function DaftarKegiatan() {
    ...
    const getKegiatan = async () => {
        try {
            const { data } = await state.db.from("kegiatan").select("id, nama")
    ...
    const [kegiatan] = createResource(getKegiatan)

    return <ul class="space-y-2 font-medium">
        <For each={kegiatan()} fallback={<SkeletonKegiatan />}>
            {item => <li>
